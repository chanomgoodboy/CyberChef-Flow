...
const NAME = 'Two-Square Cipher Decode';

class TwoSquareCipherDecode extends CustomOperation {
  constructor() {
    super();
    this.name = NAME;
    this.description = 'Decodes text encrypted with the Two-Square cipher (horizontal variant).';
  ...
  run(input: string, args: any[]): string {
    ...
    for (let i = 0; i < padded.length; i += 2) {
      // Ciphertext: first char from grid1, second from grid2
      ...
      if (c1[0] === c2[0]) {
        ...
        // Different rows: swap rows (same operation reverses itself)
...
registerCustomOp(TwoSquareCipherDecode, {
  name: NAME,
  module: 'Custom',
  description: 'Two-Square cipher decode — horizontal digraph decryption with 2 keyed squares.',
...
export default TwoSquareCipherDecode;
