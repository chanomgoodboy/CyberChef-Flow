...
const NAME = 'ReverseFuck Interpreter';

/** Translate ReverseFuck to Brainfuck: swap +/- and >/<  */
...
class ReverseFuckInterpreter extends CustomOperation {
  constructor() {
    super();
    this.name = NAME;
    this.description = 'Interprets ReverseFuck — Brainfuck with + and - swapped, and > and < swapped.';
    this.infoURL = 'https://esolangs.org/wiki/ReverseFuck';
  ...
  run(input: string, args: any[]): string {
    ...
    const bf = reverseFuckToBF(program);
...
registerCustomOp(ReverseFuckInterpreter, {
  name: NAME,
  module: 'Custom',
  description: 'Interprets ReverseFuck — Brainfuck with inverted commands.',
  infoURL: 'https://esolangs.org/wiki/ReverseFuck',
  inputType: 'string',
  outputType: 'string',
  args: [
    { name: 'Mode', type: 'option', value: ['Code in input', 'Separate code and input'] },
...
export default ReverseFuckInterpreter;
