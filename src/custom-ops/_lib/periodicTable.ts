/**
 * Periodic Table of Elements — symbols and names for cipher use.
 */

export const ELEMENTS: { symbol: string; name: string; number: number }[] = [
...
/** Element symbol set for fast lookup. */
export const SYMBOL_SET = new Set(ELEMENTS.map((e) => e.symbol.toUpperCase()));

/** Map: uppercase symbol → element. */
export const SYMBOL_MAP: Record<string, typeof ELEMENTS[0]> = {};
for (const el of ELEMENTS) {
  SYMBOL_MAP[el.symbol.toUpperCase()] = el;
}

/**
 * Decompose a word into element symbols using backtracking.
 * Returns array of symbols or null if impossible.
 */
export function decomposeWord(word: string): string[] | null {
  const upper = word.toUpperCase();
  const memo = new Map<number, string[] | null>();

  function solve(pos: number): string[] | null {
    if (pos === upper.length) return [];
    if (memo.has(pos)) return memo.get(pos)!;

    // Try 2-letter symbols first (more specific)
    for (const len of [2, 1]) {
      if (pos + len > upper.length) continue;
      const sub = upper.slice(pos, pos + len);
      // Check with proper case: first letter upper, rest lower
      const normalized = sub[0] + (sub.length > 1 ? sub[1].toLowerCase() : '');
      if (SYMBOL_MAP[sub]) {
        const rest = solve(pos + len);
        if (rest !== null) {
          const result = [normalized, ...rest];
          memo.set(pos, result);
          return result;
        }
      }
